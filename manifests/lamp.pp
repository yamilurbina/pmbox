...
class lamp($version='latest') {
  ...
  case $::operatingsystem {
    debian, ubuntu: {
      ...
      $mysqlpw_cmd = '/usr/bin/mysqladmin -u root password processmaker || /bin/true'
  ...
  if ($ok) {
    ...
    file { $apache_conf:
      ...
      source  => "/vagrant/files/apache2/apache2.${operatingsystem}.conf",
      require => Package[$apache],
    ...
    file { $sysconfig_conf:
      ...
      source  => "/vagrant/files/sysconfig/httpd.${operatingsystem}",
      require => Package[$apache],
      notify  => Service[$apache_srv],
    }
    ...
    file { $mysql_conf:
      ...
      source  => "/vagrant/files/mysql/my.${operatingsystem}.cnf",
      require => Package[$mysql],
